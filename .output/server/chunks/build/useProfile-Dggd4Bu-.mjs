...
const useProfile = () => {
  ...
  const updateProfile = async (profileData) => {
    ...
    try {
      ...
      const { $fetch: $fetch2 } = useNuxtApp();
      const response = await $fetch2("/api/v1/profile", {
        method: "PUT",
        body: formData
      });
  ...
  const updatePassword = async (passwordData) => {
    ...
    try {
      const { $fetch: $fetch2 } = useNuxtApp();
      const response = await $fetch2("/api/v1/profile/password", {
        method: "PUT",
        body: passwordData
      });
      return response;
  ...
  const uploadAvatar = async (file) => {
    ...
    try {
      ...
      const { $fetch: $fetch2 } = useNuxtApp();
      const response = await $fetch2("/api/v1/profile/avatar", {
        method: "POST",
        body: formData
      });
      if (user.value) {
...
//# sourceMappingURL=useProfile-Dggd4Bu-.mjs.map
